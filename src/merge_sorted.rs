        ...
        F: FnMut(&O,&O) -> Ordering,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(x) = &self.curr {
            ...
            if let Some(y) = &next {
                if Ordering::Greater == (self.comparator)(x, y) {
                    mem::swap(&mut self.curr, &mut next);
                    self.is_iter1_smaller = !self.is_iter1_smaller;
                }
                next
        ...
        K: Ord,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(x) = &self.curr {
            ...
            if let Some(y) = &next {
                if Ordering::Greater == (self.key)(x).cmp(&(self.key)(y)) {
                    mem::swap(&mut self.curr, &mut next);
                    self.is_iter1_smaller = !self.is_iter1_smaller;
                }
                next
...
pub fn merge_sorted<I,J,O>(iter1: I, mut iter2: J) -> MergeSorted<I,J,O> 
    where 
        I: Iterator<Item = O>,
        J: Iterator<Item = O>,
        O: Ord
...
pub fn merge_sorted_by<I,J,F,O>(iter1: I, mut iter2: J, comparator: F) -> MergeSortedBy<I,J,F,O> 
    where 
        I: Iterator<Item = O>,
        J: Iterator<Item = O>,
        F: FnMut(&O,&O) -> Ordering,
...
pub fn merge_sorted_by_key<I,J,F,O,K>(iter1: I, mut iter2: J, key: F) -> MergeSortedByKey<I,J,F,O> 
    where 
        I: Iterator<Item = O>,
        J: Iterator<Item = O>,
        F: FnMut(&O) -> K,
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.data.is_empty() {
            ...
            if let Some(mut item) = next {
                ...
                let mut i = 0;
                while i<self.data.len()-1 && self.data[i].0 > self.data[i+i].0 {
...
pub fn merge_sorted_dyn<T, O>(data: Vec<Box<dyn Iterator<Item = O>>>) -> MergeSortedDyn<O> 
    where 
